(function(){
  ...
  function companyRow(name,url){if(!name)return'';const safe=webUrl(url);return`<dt>회사</dt><dd>${safe?`<a href="${U.esc(safe)}" target="_blank" rel="noopener noreferrer">${U.esc(name)}</a>`:U.esc(name)}</dd>`}
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    try{
      ...
      if(news.length)html+='<section class="section profile-news"><div class="section-heading"><h2 class="section-title">최근 동문소식</h2><a href="news.html">전체 동문소식 →</a></div><div class="cards news-list">'+news.slice(0,5).map(NewsUI.card).join('')+'</div></section>';
